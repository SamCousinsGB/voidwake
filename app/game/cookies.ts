...
export const COOKIE_CHUNK_SIZE = 2800;
const MAX_CHUNKS = 4;
...
export function encodeSave(state: unknown) { return LZString.compressToEncodedURIComponent(JSON.stringify(state)); }
export function decodeSave(encoded: string): unknown {
  ...
  if (!raw || raw.length > 50000) throw new Error('Invalid save.');
