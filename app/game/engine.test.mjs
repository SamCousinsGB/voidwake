...
const atStation=()=>{const g=new Universe(undefined,{dynamic:false});g.s.x=STATION.x-100;g.s.y=STATION.y;g.dock();return g;};

test('manual thrust accelerates, steering turns and brakes stop the ship',()=>{
 const g=new Universe(undefined,{dynamic:false});g.keys.add('w');step(g,2);assert.ok(Math.hypot(g.s.vx,g.s.vy)>180);const before=g.s.angle;g.keys.clear();g.keys.add('a');step(g,.5);assert.ok(g.s.angle>before+1);g.keys.clear();g.keys.add('s');step(g,2);assert.ok(Math.hypot(g.s.vx,g.s.vy)<1);
});
test('autopilot actually flies to the station and docks',()=>{
 const g=new Universe(undefined,{dynamic:false});let opened=false;g.onDock=()=>opened=true;g.dock();assert.equal(g.autoDock,true);step(g,12);assert.equal(g.docked,true);assert.equal(opened,true);assert.equal(g.s.vx,0);g.undock();assert.equal(g.docked,false);assert.ok(Math.hypot(g.s.x-STATION.x,g.s.y-STATION.y)<185);
});
test('market enforces credits, capacity, integer quantity, cargo and station access',()=>{
 const g=new Universe(undefined,{dynamic:false});assert.equal(g.trade(0,5,true).ok,false);g.s.x=STATION.x;g.s.y=STATION.y;g.dock();const before=g.s.credits;assert.equal(g.trade(0,5,true).ok,true);assert.equal(g.s.credits,before-210);assert.equal(g.s.cargo[0],5);assert.equal(g.stock[0],75);assert.equal(g.trade(0,31,true).ok,false);assert.equal(g.trade(2,20,true).ok,false);assert.equal(g.trade(0,-1,true).ok,false);assert.equal(g.trade(0,.5,true).ok,false);assert.equal(g.trade(6,1,true).ok,false);assert.equal(g.trade(0,6,false).ok,false);assert.equal(g.trade(0,5,false).ok,true);assert.equal(g.s.cargo[0],0);assert.ok(g.s.credits<before,'buy-sell spread must prevent local arbitrage');
});
test('travel consumes exact fuel once, visits new system and preserves cargo',()=>{
 const g=new Universe(undefined,{dynamic:false});g.s.cargo[0]=8;const fuel=g.s.fuel;assert.equal(g.startJump(5),false,'far system requires intermediate jump');assert.equal(g.startJump(1),true);assert.equal(g.startJump(2),false,'only one jump can be queued');step(g,4);assert.equal(g.s.system,1);assert.equal(g.s.fuel,fuel-fuelCost(0,1));assert.equal(g.s.cargo[0],8);assert.deepEqual(g.s.visited,[0,1]);assert.equal(g.jump,null);assert.equal(g.startJump(1),false);g.s.fuel=0;assert.equal(g.startJump(0),false);
});
...
test('phaser array hits targeted hostiles, grants bounty and creates salvage',()=>{
 const g=new Universe(undefined,{dynamic:false});g.s.x=800;g.s.y=0;g.contacts=g.contacts.filter(c=>c.kind==='station');g.contacts.push({id:'pirate-0',name:'Target',kind:'hostile',x:1050,y:0,vx:0,vy:0,angle:0,hull:80,maxHull:80,shield:0,fire:99});g.target='pirate-0';g.keys.add(' ');const before=g.s.credits;step(g,5);assert.equal(g.s.kills,1);assert.equal(g.s.credits,before+240);assert.ok(g.contacts.some(c=>c.kind==='salvage'));assert.ok(!g.contacts.some(c=>c.id==='pirate-0'));assert.ok(g.s.energy>=0);
});
test('hostile shots deplete shields then damage the hull; shields regenerate',()=>{
 const g=new Universe(undefined,{dynamic:false});g.s.x=800;g.s.y=0;g.s.shield=10;g.sinceHit=0;const hull=g.s.hull;g.bullets.push({id:1,x:g.s.x,y:g.s.y,vx:0,vy:0,life:1,enemy:true,damage:20});g.update(.01);assert.equal(g.s.shield,0);assert.equal(g.s.hull,hull-10);g.contacts=g.contacts.filter(c=>c.kind==='station');step(g,7);assert.ok(g.s.shield>15);assert.equal(g.s.hull,hull-10);
});
...
test('pause freezes physics and jump countdown',()=>{
 const g=new Universe(undefined,{dynamic:false});g.startJump(1);g.paused=true;const before=JSON.stringify(g.s);step(g,10);assert.equal(JSON.stringify(g.s),before);assert.equal(g.jumpTime,3);g.paused=false;step(g,4);assert.equal(g.s.system,1);
});
