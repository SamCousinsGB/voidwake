...
export default function SpaceView({game,onTick}:{game:Universe;onTick:()=>void}){
 ...
 useEffect(()=>{
  ...
  const stars=new Float32Array(6500*3),starColors=new Float32Array(6500*3);for(let i=0;i<6500;i++){stars[i*3]=(random()-.5)*9000;stars[i*3+1]=(random()-.5)*9000;stars[i*3+2]=-200-random()*500;const v=.3+random()*.7;starColors[i*3]=v*.72;starColors[i*3+1]=v*.85;starColors[i*3+2]=v;}
  const starGeo=new THREE.BufferGeometry();starGeo.setAttribute('position',new THREE.BufferAttribute(stars,3));starGeo.setAttribute('color',new THREE.BufferAttribute(starColors,3));scene.add(new THREE.Points(starGeo,new THREE.PointsMaterial({size:1.4,vertexColors:true,sizeAttenuation:false,transparent:true,opacity:.8})));
  const nebula=new THREE.Mesh(new THREE.PlaneGeometry(11000,11000),new THREE.ShaderMaterial({transparent:true,depthWrite:false,vertexShader:'varying vec2 vUv;void main(){vUv=uv;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}',fragmentShader:`varying vec2 vUv;${noiseGLSL}void main(){float f=fbm(vec3(vUv*13.,3.4));float m=fbm(vec3(vUv*22.,2.));vec3 c=mix(vec3(.026,.075,.12),vec3(.12,.045,.13),m);gl_FragColor=vec4(c,pow(f,3.)*1.6);}`}));nebula.position.z=-900;scene.add(nebula);
  ...
  let last=performance.now(),uiElapsed=0,raf=0;function frame(now:number){const dt=Math.min((now-last)/1000,.05);last=now;game.update(dt);if(renderedSystem!==game.s.system)rebuild();const s=game.s;
   camera.position.x+=(s.x-camera.position.x)*Math.min(dt*3,1);camera.position.y+=(s.y+90-camera.position.y)*Math.min(dt*3,1);const span=950/game.zoom;camera.left=-span*width/height/2;camera.right=-camera.left;camera.top=span/2;camera.bottom=-span/2;camera.updateProjectionMatrix();camera.updateMatrixWorld();
