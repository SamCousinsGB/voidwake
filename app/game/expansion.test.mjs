...
const quiet=()=>{const g=new Universe();g.contacts=g.contacts.filter(c=>c.kind==='planet'||c.kind==='station');return g;};
...
test('24 populated systems contain 120 uniquely named, selectable worlds and six complete fleets',()=>{
 assert.equal(SYSTEMS.length,24);assert.equal(Object.keys(SHIPS).length,24);assert.equal(factionIds.length,6);
 const ids=new Set(),names=new Set();for(const sys of SYSTEMS){const g=new Universe();g.s.system=sys.id;g.populate();assert.equal(g.worlds.length,5);assert.equal(g.contacts.filter(c=>c.kind==='planet').length,5);assert.ok(g.contacts.filter(c=>c.kind==='trader').length>=5);for(const w of planetsFor(sys.id)){ids.add(w.id);names.add(w.name);assert.ok(Number.isFinite(w.x)&&Number.isFinite(w.y));assert.equal(g.hail(w.id),true);assert.equal(g.interactionContact.id,w.id);}}
 assert.equal(ids.size,120);assert.equal(names.size,120);for(const f of factionIds){assert.equal(fleetFor(f).length,4);assert.equal(SYSTEMS.filter(s=>s.factionId===f).length,4);}
});
...
test('torpedoes travel over time and preferentially deplete shields',()=>{
 const g=quiet(),c=dummy('torpedo-target','trader',500,0);g.contacts=[c];c.shield=180;g.s.x=0;g.s.y=0;g.target=c.id;g.setWeapon('torpedo');g.fire();assert.equal(c.shield,180);assert.equal(g.bullets[0].kind,'torpedo');c.kind='station';c.fire=999;step(g,1.5);assert.ok(c.shield<180-g.stats.damage*WEAPONS.torpedo.multiplier);assert.equal(c.hull,1000);
});
...
test('a fully explored expanded campaign including conflicts fits verified cookie saves',()=>{
 const g=new Universe();g.s.ship='citadel';g.s.time=125000;g.s.credits=1500000;g.s.upgrades={weapon:3,shield:3,engine:3,cargo:3};g.s.visited=SYSTEMS.map(s=>s.id);
 ...
 const state=Universe.validate(g.s);assert.ok(state);const encoded=encodeSave(state);assert.ok(encoded.length<=11200,encoded.length+' encoded chars');assert.ok(JSON.stringify(state).length<=50000);assert.deepEqual(decodeSave(encoded),state);
